...
sys.path.append('./python-veml6070')
# sys.path.append('./')
...
try:
    ...
    print("TSL2591 Init Complete: LUX: %f W/(m*m),FULL: %d, IR: %d" % (lux, full, ir))
    # print (lux, full, ir)
    # print ()
...
try:
    config.VEML6070_Present = False
    #restorePower(SI1145GSPIN)
    # time.sleep(1.0)
    # VEML6070_Sensor = SDL_Pi_SI1145.SDL_Pi_SI1145(indoor=0)
    # time.sleep(1.0)
    # veml = veml6070.Veml6070()
    # veml.set_integration_time(veml6070.INTEGRATIONTIME_1T)
    # i = veml6070.INTEGRATIONTIME_1T
    # uv_raw = veml.get_uva_light_intensity_raw()
    # uv = veml.get_uva_light_intensity()
    # uvi = get_estimated_risk_level(uv)
    # visible = veml.get_uva_light_intensity_raw() 
    # print "visible=", visible
    # config.VEML6070_Present = False
    # vis = veml.get_uva_light_intensity_raw()
    # IR = vis
    # UV = veml.get_uva_light_intensity()
    # IR_Lux = UV
    # vis_Lux = UV
    # uvIndex = UV / 100.0
    # print("Integration Time setting %d: %f W/(m*m) from raw value %d give uvi value of %s" % (i, UV, vis, uvIndex))
...
def returnStatusLine(device, state):
    returnString = device
    if (state == True):
        returnString = returnString +": \033[1;32;40m Present"
    else:
        returnString = returnString +": \033[1;31;40m Not Present\033[0;0m"
    return returnString
...
I2C_Lock = threading.Lock()
    ...
    # named for SunAirPlus Solar Power 
...
SDL_MODE_I2C_ADS1015 = 1    # internally, the library checks for ADS1115 or ADS1015 if found
    ...
    # THe wind speed is averaged at sampleTime
    # or when you ask, whichever is longer
...
try:
    conf={'ll':{'type':'rfm95'},'pl':{'type':'serial_seed','port':'/dev/ttyS0'}}
...
if (config.SHT30_Present == False):  # don't check for AM2315 if you find SHT30
    ...
    # write SunAirPlus stats  #

def writeSunAirPlusStats():
    f = open("./state/SunAirPlusStats.txt", "w")
...
def writeWeatherStats():
    f = open("./state/WeatherStats.txt", "w")
...
def sampleWeather():
    ...
    if (config.BMP280_Present):
        try:
            ...
            print("BMP280 Unexpected error:", sys.exc_info()[0])
    ...
    if (config.BME680_Present):
        try:
            ...
            print("BME680 Unexpected error:", sys.exc_info()[0])
    ...
    elif (config.VEML6070_Present):
        veml.set_integration_time(veml6070.INTEGRATIONTIME_1T)
        SunlightUV = veml.get_uva_light_intensity()
        SunlightUVIndex = SunlightUV / 100.0

    else:
        # no sunlight sensors present set values to zero
        SunlightVisible = -1
        SunlightIR = -1 
        SunlightUV = 0
        SunlightUVIndex = 0.
    ...
    if (config.WXLink_Present == False) :
        # do not use internal AM2315 or SHT30 if we are WXLink connected
        ...
        if (config.SHT30_Present):
            ...
            if (crc_checkH !=  -1) and (crc_checkT != -1):
                ...
                state.currentOutsideHumidity = outsideHumidity
            if (config.SWDEBUG == True):
                print ("SHT30 Stats1: (g,br,bc,rt,pc)", sht30.read_status_info())
    else:
        # WXLink == True if SolarMAX is here and WXLink is not, the read AM2315/SHT30
        ...
        if (config.SHT30_Present):
            ...
            if (crc_checkH !=  -1) and (crc_checkT != -1):
                ...
                state.currentOutsideHumidity = outsideHumidity
            if (config.SWDEBUG == True):
                print ("SHT30 Stats2: (g,br,bc,rt,pc)", sht30.read_status_info())
    ...
    if (config.WeatherUnderground_Present == True):
        ...
        try:
            ...
            WeatherUnderground.sendWeatherUndergroundData( config.WeatherUnderground_StationID, config.WeatherUnderground_StationKey, as3935LightningCount, as3935, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain, bmp180Temperature, bmp180SeaLevel, bmp180Altitude,  bmp180SeaLevel, state.currentOutsideTemperature, state.currentOutsideHumidity, crc_check, currentWindDirection, currentWindDirectionVoltage, HTUtemperature, HTUhumidity, rain60Minutes, SunlightUVIndex)
            # WeatherUnderground.sendWeatherUndergroundData( config.WeatherUnderground_StationID, config.WeatherUnderground_StationKey, as3935LightningCount, as3935, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, totalRain, bmp180Temperature, bmp180SeaLevel, bmp180Altitude,  bmp180SeaLevel, state.currentOutsideTemperature, state.currentOutsideHumidity, crc_check, HTUtemperature, HTUhumidity, rain60Minutes, SunlightUVIndex)
        except:
...
print (returnStatusLine("VEML6070 UV Sensor     ",config.VEML6070_Present))
...
if (config.enable_MySQL_Logging == True):
    scheduler.add_job(writeWeatherRecord, 'interval', seconds=1*60)
...
while True:
    if (as3935Interrupt == True):
        ...
        try:
            ...
            print ("I/O error({0}): {1}".format(e.errno, e.strerror))
            print ("exception - as3935 I2C error")
        I2C_Lock.release()
    # if (config.TCA9545_I2CMux_Present):
    #    tca9545.write_control_register(TCA9545_CONFIG_BUS0)
    time.sleep(1.0)
